...
/**
 * Returns wrapper around RequestError constructor which presets HTTP status
 * code allowing only for message param.
...
/**
 * Request errors reflect HTTP status codes, in particular 4xx codes (client
 * errors) and 5xx codes (server errors). This constructor inherits from Error
 * constructor with few differences. Firstly it takes different amount of
 * arguments and all are optional. Secondly it provides additional properties
 * and exposes name property (it is enumerable compared to Error).
 *
 * If status code is not provided it defaults to 500 Internal Server Error.
 * If status code is not a valid HTTP status code then RequestError name
 * property is set to "RequestError". If no message is provided then it
 * defaults to RequestError name property.
 *
 * @param [status] {Number|*} HTTP status code. Error name is based on the
 * status code.
 * @param [message] {String|Error} Message string or valid Error instance.
 * If Error instance is provided then message is set using Error.message
 * property, and Error is stored internally.
