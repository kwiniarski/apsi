...
function Resource(model, controller, policies) {
  ...
  if (model) {
    blueprint = new ResourceActionBlueprint(model, this.mountPath);
  }
...
Resource.prototype.registerPolicies = function (policies) {
  this.policies = policies.get(this.name);
  this.actions.forEach(function (action) {
    action.registerPolicies(policies.get(this.name, action.name));
  }.bind(this));
};

Resource.prototype.registerPolicy = function (policy, actions) {
  if (actions && actions instanceof Array) {
    actions.forEach(function (actionName) {
      this.actions[actionName].registerPolicy(policy);
    }.bind(this));
  } else {
