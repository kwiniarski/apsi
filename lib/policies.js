...
function getPolicy(policyName) {
  if (policyFiles[policyName]) {
    return policyFiles[policyName];
  } else if (policyFiles[support.camelCaseToDash(policyName)]) {
    return policyFiles[support.camelCaseToDash(policyName)];
  } else {
    var error = new Error('Unknown policy "' + policyName + '"');
    throw error;
  }
}
